...
import type { LifeItem, Transaction, Subscription, SettingsState } from '@/lib/types';
...
function rowToLifeItem(r: DbLifeItem): LifeItem {
  return {
    ...
    notificationId: r.notification_id != null ? String(r.notification_id) : null,
  };
}
...
function rowToSubscription(r: DbSubscription): Subscription {
  return {
    ...
    aiMeta: r.ai_meta != null && typeof r.ai_meta === 'object' && 'confidence' in r.ai_meta
      ? { confidence: Number((r.ai_meta as { confidence: unknown }).confidence) }
      : undefined,
  };
}
...
function rowToSettings(r: DbSettings): SettingsState {
  return {
    ...
    defaultRemindDaysBefore: (r.default_remind_days_before === 7 || r.default_remind_days_before === 14 || r.default_remind_days_before === 30)
      ? r.default_remind_days_before
      : 7,
  };
}
...
export async function updateLifeItem(id: string, patch: Partial<LifeItem>): Promise<void> {
  ...
  if (patch.amountCents !== undefined) row.amount_cents = patch.amountCents ?? null;
  ...
  if (patch.remindDaysBefore != null) row.remind_days_before = patch.remindDaysBefore;
  if (patch.status != null) row.status = patch.status;
  if (patch.notes !== undefined) row.notes = patch.notes ?? null;
  if (patch.notificationId !== undefined) row.notification_id = patch.notificationId ?? null;
  if (Object.keys(row).length === 0) return;
  const { error } = await supabase.from('life_items').update(row).eq('id', id).eq('user_id', uid);
  if (error) throw error;
}
...
export async function deleteLifeItem(id: string): Promise<void> {
  const uid = await getUserId();
  if (!supabase || !uid) throw new Error('Not authenticated');
  const { error } = await supabase.from('life_items').delete().eq('id', id).eq('user_id', uid);
  if (error) throw error;
}
...
export async function setSubscriptions(subs: Subscription[]): Promise<void> {
  const uid = await getUserId();
  if (!supabase || !uid) throw new Error('Not authenticated');
  const { error: delErr } = await supabase.from('subscriptions').delete().eq('user_id', uid);
  if (delErr) throw delErr;
