...
export default function TodayScreen() {
  ...
  const [viewDateISO, setViewDateISO] = React.useState<string>(() =>
    todayISO(),
  );
  ...
  const [reviewParsed, setReviewParsed] = React.useState<
    import("@/lib/ai/schemas").SmartInputParseResult | null
  >(null);
  ...
  // Upcoming money: items only (no subscriptions), same as money tab breakdown from real data only
  const upcoming30 = computeUpcomingBreakdown(activeItems, [], 30);
  const upcoming7 = computeUpcomingBreakdown(activeItems, [], 7);
  ...
  // Reminder analytics (in At a glance)
  const completedCount = items.filter((i) => i.status === "completed").length;
  const overdueCount = activeItems.filter((i) =>
    isOverdue(i.nextDueISO, i.dueTime),
  ).length;
  ...
  const handleUpdateItem = async (id: string, patch: Partial<LifeItem>) => {
    ...
    const { scheduleDueReminder, cancelScheduledNotification } =
      await import("@/lib/notifications");
    const existing = await getLifeItem(id);
    if (!existing) return;
    const becomingCompletedOrCancelled =
    ...
    if (!becomingCompletedOrCancelled && remindDays > 0) {
      ...
      notificationId =
        (await scheduleDueReminder(
          ...
          nextDue,
          remindDays,
          patch.dueTime ?? existing.dueTime ?? undefined,
          patch.remindMinutesBefore ?? existing.remindMinutesBefore ?? 30,
        )) ?? undefined;
    }
    ...
    const updated = items.map((i) =>
      i.id === id ? { ...i, ...patch, notificationId } : i,
    );
      ...
      <View style={[styles.container, { backgroundColor: theme.background }]}>
        ...
        >
          ...
          <View style={[styles.statusCard, { backgroundColor: theme.surface }]}>
            <Text
              style={[styles.statusCardTitle, { color: theme.textSecondary }]}
            >
              Your load
            </Text>
            <Text
              style={[styles.statusCardHint, { color: theme.textTertiary }]}
            >
              {lifeStatus === "Stable" &&
                "You’re on top of things — few or no reminders due in the next 7 days."}
              {lifeStatus === "Watch" &&
                "A few reminders need attention soon. Consider planning or paying in the next 7 days."}
              {lifeStatus === "Action Needed" &&
                "Several reminders are due soon. Plan or pay to stay on track."}
            </Text>
            <View
              style={[
                styles.statusChip,
                {
                  backgroundColor: statusColors[lifeStatus],
          ...
          <View style={styles.comingUpHeader}>
            ...
            >
              <Text style={[styles.viewAllLink, { color: theme.tint }]}>
                View all reminders
              </Text>
            </TouchableOpacity>
          </View>
          <View style={styles.filterRow}>
            <TouchableOpacity
              style={[
                ...
                {
                  backgroundColor:
                    comingUpDays === 0 ? theme.accentPill : theme.pillBg,
                },
              ]}
            ...
            >
              <Text
                style={[
                  ...
                  {
                    color:
                      comingUpDays === 0 ? theme.text : theme.textSecondary,
                  },
                ]}
              >
                Today
            ...
            {([7, 14, 30] as const).map((days) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  {
                    backgroundColor:
                      comingUpDays === days ? theme.accentPill : theme.pillBg,
                  },
                ]}
          ...
          <TouchableOpacity
            activeOpacity={0.8}
            onPress={() =>
              upcoming30.byCategory.length > 0 && setUpcomingExpanded((e) => !e)
            }
          ...
          >
            <View style={styles.upcomingHeadlineRow}>
              <Text style={[styles.upcomingHeadline, { color: theme.text }]}>
                You have {formatCurrency(upcoming30.totalCents)} due in the next
                30 days
              </Text>
              {upcoming30.byCategory.length > 0 && (
                <Text
                  style={[
                    styles.upcomingTapHint,
            ...
            {upcomingExpanded && upcoming30.byCategory.length > 0 && (
              <View style={styles.breakdown}>
                {upcoming30.byCategory.map(
                  ({ category, amountCents }, index) => (
                    <View
                      key={category}
                      style={[
                        styles.breakdownRow,
            ...
            {!upcomingExpanded && upcoming30.byCategory.length === 0 && (
              <Text
                style={[styles.upcomingHint, { color: theme.textTertiary }]}
              >
                Add reminders or subscriptions with amounts to see your
                forecast.
              </Text>
            )}
          </TouchableOpacity>
          ...
          <Text style={[styles.glanceSubtitle, { color: theme.textTertiary }]}>
            Reminders
          </Text>
          <View style={[styles.glanceGrid, styles.glanceGridTight]}>
            <View
              style={[styles.glanceCard, { backgroundColor: theme.surface }]}
            >
              <Text
                style={[styles.glanceLabel, { color: theme.textSecondary }]}
              >
                Completed
              </Text>
            ...
            >
              ...
              <Text
                style={[
                  styles.glanceValue,
                  { color: overdueCount > 0 ? theme.chipAction : theme.text },
                ]}
              >
                {overdueCount}
              </Text>
          ...
          <Text style={[styles.glanceSubtitle, styles.glanceSubtitleAfter, { color: theme.textTertiary }]}>
            Spending
          </Text>
          <View style={styles.glanceGrid}>
            ...
            >
              ...
              <Text style={[styles.glanceValue, { color: theme.text }]}>
                {formatCurrency(upcoming30.totalCents)}
              </Text>
            </View>
            <View
              style={[styles.glanceCard, { backgroundColor: theme.surface }]}
            >
              ...
              <Text style={[styles.glanceValue, { color: theme.text }]}>
                {formatCurrency(upcoming7.totalCents)}
              </Text>
            </View>
            <View
              style={[styles.glanceCard, { backgroundColor: theme.surface }]}
